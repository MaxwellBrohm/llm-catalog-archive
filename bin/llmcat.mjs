...
export function looksLikeCatalogId(value) {
 ...
 */
export function retiringRows(retirements, wanted, windowDays, today) {
 ...
 */
export function coveredProviders(retirements) {
    const seen = new Set();
 ...
 */
export async function readDoc(base, rel) {
...
`;
async function cmdModels(args, base, out) {
    ...
    out(table([
        ...
        ...models.map((m) => {
            ...
            const top = m['top_provider_context_length'];
            return [
                str(m, 'id'),
...
async function cmdWatch(args, base, out) {
    ...
    if (!found) {
        ...
        if (!looksLikeCatalogId(id))
            out('Model ids in this catalog are vendor-prefixed, for example anthropic/claude-opus-5.');
        return 1;
    }
    if (api === null) {
...
async function cmdPriceHistory(args, base, out) {
    const id = args.positionals[0];
    if (id === undefined) {
...
async function cmdLeaks(args, base, out) {
    ...
    for (const r of refusals)
        out(`  ${str(r, 'source_id')} at ${str(r, 'sha').slice(0, 7)}: ${str(r, 'reason')}`);
    return 0;
}
/** The model names to check: --models, --file, or every floor in the archive. */
async function wantedModels(args) {
    const inline = args.flags.get('models');
    const file = args.flags.get('file');
    const names = [];
    if (typeof inline === 'string')
        names.push(...inline.split(','));
    if (typeof file === 'string')
        names.push(...(await fs.readFile(file, 'utf8')).split('\n'));
    return names.map((n) => n.trim()).filter((n) => n !== '');
}
async function cmdRetiring(args, base, out) {
    ...
    const retirements = asArray(doc, 'retirements');
    ...
    if (args.flags.has('json')) {
        ...
        if (rows.some((r) => r.status === 'unknown' || r.status === 'undated'))
            return 2;
    ...
    if (other.length > 0) {
        ...
        for (const r of other) {
            if (r.status === 'outside')
                out(`  ${r.model}: floor ${r.record?.floor_date ?? ''}, ${String(r.days)} day(s) out`);
            else
                out(`  ${r.model}: ${r.note ?? r.status}`);
        }
    }
    const source = doc['source'];
    if (source !== null) {
