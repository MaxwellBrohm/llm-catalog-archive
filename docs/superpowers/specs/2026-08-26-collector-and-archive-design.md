...
| Source | URL | Notes |
|---|---|---|
| `arena-leaderboard` | `https://arena.ai/leaderboard` | Section 7. Hourly during the O4 measurement window. **Parked 2026-09-26** (status `pending`): arena replaced the 5.2 MB `publicName`-keyed payload with a 773,495-byte React Server Components stream keyed on `modelKey`, measured live with 0 occurrences of `publicName` and a size ratio of 0.153 against a `[0.5, 2.0]` band. Both guards were right to refuse it. Superseded by `arena-leaderboard-rsc`. |
| `arena-leaderboard-rsc` | `https://arena.ai/leaderboard` | 773,495 bytes measured 2026-09-26, the same page in its RSC form. Same predicate: `arenaRows` in `any` mode reads `modelKey` as well as `publicName` and yields 552 records against the 500 floor, all 552 carrying both a rating and a vote count where the old page left 89 of 942 rows with neither. A NEW id rather than a re-pointing, so its first capture is a baseline and rule 2 bars events from it. |
...
| `openrouter-sitemap` | `https://openrouter.ai/sitemap.xml` | 616,687 bytes, 5,051 `<url>`. **Not static:** rebuilt several times a day, rewriting ~100 `<lastmod>` values per rebuild independent of content change. Predicate keys on the `<loc>` set only. **Parked 2026-09-26** (status `pending`): the URL now returns a 1,124-byte `<sitemapindex>` of per-section sub-sitemaps, so `expectedRoot: urlset` can never match it again. Superseded by `openrouter-models-sitemap`. |
| `openrouter-models-sitemap` | `https://openrouter.ai/sitemaps/models.xml` | 124,911 bytes measured 2026-09-26. The model half of the split sitemap, which is the only section the `sitemapLoc` predicate ever read; the others (static, authors, providers, collections, rankings, compare) are navigation. A NEW id rather than a re-pointing, so no "5,242 URLs vanished" can be derived from a reorganisation. |
...
| `mistral-llms-txt` | `https://docs.mistral.ai/llms.txt` | 14,658 bytes at first capture; 55,752 measured 2026-09-26 after a month of real product launches (Le Chat, Vibe, Mistral AI Studio), a ratio of 3.804 that the `[0.5, 2.0]` band refused. Band widened to `[0.5, 4.5]` for this source only, after reading the body to confirm it is genuine. Canary corrected from `# MistralAI`, which occurs zero times, to `mistral.ai`, which occurs 287. |
...
| `xai-llms-txt` | `https://docs.x.ai/llms.txt` | 1,465,407 bytes at spec time; **17,023 measured 2026-09-26**, the doc having become a short index that points at per-page `.md` files. Row-permuting, see section 7. No `llms-full` exists. 18 consecutive failures and NOT ONE stored capture under the old floor, so correcting it in place cannot produce a false event. Canary corrected from `https://api.x.ai/v1`, which occurs zero times, to `docs.x.ai`, which occurs 176. |
...
| `transformers-pulls` | `https://api.github.com/search/issues?q=repo:huggingface/transformers+is:pr+model+support+in:title&sort=created&order=desc&per_page=100` | Leaks desk, signal 2. Probed live 2026-08-31: 200, 483,237 bytes, `total_count` 203, 100 items back to 2024-09-20. The search API is a **separate rate-limit bucket** from core REST, 10/minute unauthenticated, which one daily call does not approach. **`merged_at` is present in the search payload itself**, on `items[].pull_request.merged_at`, so distinguishing merged from abandoned does NOT require a second `/pulls/<n>` fetch. Predicate `extracted`/`githubPulls`. |
| `vllm-pulls` | `https://api.github.com/search/issues?q=repo:vllm-project/vllm+is:pr+model+support+in:title&sort=created&order=desc&per_page=100` | The other half of signal 2. Probed live 2026-08-31: 200, 657,623 bytes, `total_count` 1043, 100 items back only to 2026-07-16, so its window is six weeks against the transformers window's two years. Predicate `extracted`/`githubPulls`. |
| `deepmind-sitemap` | `https://deepmind.google/sitemap.xml` | **An announcement index, not a documentation one.** Added 2026-09-01 to close the gap between what the brief asked for, a general AI news site, and what the archive derived, which was API-surface changes only. 85,116 bytes, 727 `<loc>`, of which 347 under `/blog/` and 269 under `/research/`. A real `<urlset>`, not a sitemap index. Carries no per-URL `lastmod`, so the predicate is `extracted`/`sitemapLoc` rather than `sitemapDated`: there is no edit signal to key on. Activation gate passed on two fetches 20 seconds apart, byte-identical bodies and an identical `sitemapLoc` projection. |
| `openai-sitemap` | `https://openai.com/sitemap.xml/research/` | **The research SUB-sitemap, not the root.** `openai.com/sitemap.xml` is a sitemap INDEX whose children are urlsets, and a collector pointed at the root would archive a list of other sitemaps. This child carries the announcement posts: 408,127 bytes, 46 `<loc>`, all under `/index/`, which is where OpenAI publishes. Predicate `extracted`/`sitemapLoc` for the same reason as `deepmind-sitemap`. Activation gate passed the same way. |
| `openai-news-feed` | `https://openai.com/news/rss.xml` | **The first source carrying a provider's own HEADLINES rather than URLs alone.** 1,159 items, RSS 2.0, 703,109 bytes. Added 2026-09-01 so the publication reports news a person can read rather than "a URL appeared". Predicate `extracted`/`feedPosts`: the channel carries a `<lastBuildDate>` that re-stamps on every rebuild, so `bytes` would commit 700 KB every run for a feed whose posts had not changed. |
| `deepmind-blog-feed` | `https://deepmind.google/blog/rss.xml` | 100 items, RSS 2.0, 74,065 bytes, with real headlines. Complements `deepmind-sitemap`, which carries 727 URLs and no titles: where a provider offers both, the feed is the better artifact and the sitemap stays for coverage. Predicate `extracted`/`feedPosts`. |
...
| `mistral-news-feed` | `https://mistral.ai/news/rss` | **A major lab that was missing because its advertised feed URL is wrong.** `mistral.ai/news/rss.xml` returns 404; `rss.xml` at the root 302s to `news/rss`, which is the canonical address and the one configured here, found by probing rather than by trusting the documented path. 82 items, RSS 2.0, 22,983 bytes, newest 8 days old, 26-day maximum gap. Predicate `extracted`/`feedPosts`. Activation gate passed. |
...
| `pytorch-blog-feed` | `https://pytorch.org/blog/feed/` | The framework almost all of this is trained and served on. 10 items, RSS 2.0, 174,110 bytes, newest 4 days old with a 7-day maximum gap, the tightest cadence of any source here. Predicate `extracted`/`feedPosts`. Activation gate passed. |
